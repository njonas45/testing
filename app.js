const API_BASE = "https://ssl.students.engr.scu.edu/~njonas/cgi-bin";
...
function showView(view, btn) {
    ...
    ['listings', 'watchlist', 'history', 'log'].forEach(v => {
...
function renderLog() {
    ...
    if (market.logs.length === 0) {
        el.innerHTML = `<div class="empty"><p>No activity yet.</p></div>`;
        return;
    }
    // Show most recent entries first
    el.innerHTML = [...market.logs].reverse().map(entry => `
        <div class="log-row">
            <div class="log-text">${entry}</div>
        </div>`).join('');
}
...
async function renderListings() {
    ...
    document.getElementById('stat-bids').textContent =
        all.reduce((sum, x) => sum + x.bids.length, 0);
...
function renderHistory() {
    ...
    el.innerHTML = [...entries].reverse().map(item => `
        <div class="list-row">
          <span class="status-badge ${item.win ? 'status-won' : 'status-active'}">
            ${item.win && item.winner === currentUser().username ? 'Won' : 'Bid'}
          </span>
          <div class="list-row-info">
            <div class="row-title">${item.item_name}</div>
            <div class="row-meta">Highest bid: ${fmt(item.highestBid)}</div>
          </div>
        </div>`).join('');
}
